...
class Job(models.Model):
    ...
    CATEGORIES = ['photography', 'architechture', 'music', 'theatre', 'receptionist',
                  ...
                  'marketing', 'computer_science', 'software_engineering',
                  'mechanical_engineering', 'electrical_engineering', 'industrial_engineering',
                  'tutor', 'elementary_school_teacher', 'highschool_teacher', 'cegep_teacher',
                  'university_teacher', 'biology', 'chemistry',
...
class Question(models.Model):
    ...
    accepted_answer_id = models.OneToOneField(
        'Answer', on_delete=models.SET_NULL, null=True,
        blank=True, related_name='accepted_answer_set')
    rejected_answers_ids = models.ManyToManyField(
        'Answer', related_name='rejected_answers_set', blank=True)
    points = models.IntegerField(default=0)
    tags = models.ManyToManyField(
        'Tag', related_name='question_set', blank=True)
...
class Profile(models.Model):
    ...
    first_name = models.CharField(
        max_length=32, default="Unset Name", null=False)
    last_name = models.CharField(
        max_length=32, default="Unset Last Name", null=False)
    ...
    upvoted_questions = models.ManyToManyField(
        Question, related_name='up_questions', blank=True)
    downvoted_questions = models.ManyToManyField(
        Question, related_name='down_questions', blank=True)
    upvoted_answers = models.ManyToManyField(
        Answer, related_name='up_answers', blank=True)
    downvoted_answers = models.ManyToManyField(
        Answer, related_name='down_answers', blank=True)
